 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_WQV_Framework_Field_select' ) ) {
	class SP_WQV_Framework_Field_select extends SP_WQV_Framework_Fields {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ! empty( $args['ajax'] ) ) {
					...
					$args['settings']['data']['nonce'] = wp_create_nonce( 'sp_wqv_chosen_ajax_nonce' );
				...
				$chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' sp_wqv-chosen-sortable' : '';
				$chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' sp_wqv-chosen-ajax' : '';
				$placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '"' : '';
				$field_class      = ( $args['chosen'] ) ? ' class="sp_wqv-chosen' . esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) . '"' : '';
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . $field_name . '" class="sp_wqv-hide-select hidden"' . $multiple_attr . $field_attr . '>';
